...
data = [segment.split("\n") for segment in data]
data = [[list(map(int, re.findall(r"\d+", part))) for part in segment] for segment in data]

tokens = 0
for segment in data:
    buttonA, buttonB, prize = segment
    a, c = buttonA
    b, d = buttonB
    x, y = prize
    i, j = Fraction(d*x-b*y)/Fraction(a*d-b*c), Fraction(a*y-c*x)/Fraction(a*d-b*c)
